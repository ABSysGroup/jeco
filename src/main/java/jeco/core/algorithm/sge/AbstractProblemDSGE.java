...
public abstract class AbstractProblemDSGE extends AbstractProblemSGE<VariableList<Integer>> {
	...
	protected Phenotype generatePhenotype(Solution<VariableList<Integer>> solution) {
		...
		//Set the number of all genes in the solution
		int acc = 0;
		
		for(VariableList<Integer> i: solution.getVariables()) {
			acc += i.size();
		}
		
		solution.setNumberGenes(acc);
    ...
    @Override
    protected Solution<VariableList<Integer>> initializeInd(){
    	Solution<VariableList<Integer>> solI = new Solution<>(numberOfObjectives);
    	ArrayList<VariableList<Integer>> temp = new ArrayList<>();
    ...
    private void constructSolutionFromTree(ArrayList<VariableList<Integer>> solI, RecListT<Integer> n) {

        solI.get(this.orderSymbols.indexOf(n.getS().toString())).add(n.getValue());
